...
// custom
    ...
    #define PREP_SUB(fncName,folder) DFUNC(fncName) = compile preprocessFileLineNumbers QPATHTOF(functions\folder\DOUBLES(fnc,fncName).sqf)
#else
    #define PREP_SUB(fncName,folder) [QPATHTOF(functions\folder\DOUBLES(fnc,fncName).sqf), QFUNC(fncName)] call CBA_fnc_compileFunction
#endif
